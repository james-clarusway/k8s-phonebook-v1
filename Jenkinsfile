    ...
    stage('deploy application') {
      steps {
        withKubeCredentials(
          ...
          sed -i 's|${ECR_REGISTRY}/${APP_REPO_NAME}:web-b${BUILD_NUMBER}|' k8s/webserver-deploy.yaml
          sed -i 's|${ECR_REGISTRY}/${APP_REPO_NAME}:reult-b${BUILD_NUMBER}' k8s/resultserver-deploy.yaml          
          kubectl apply -f k8s
          '''
        }
      }
